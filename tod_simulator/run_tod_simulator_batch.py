...
Run:
  python run_tod_simulator_batch.py --n 1000 --turns tod_sim_turns_1000.csv --metrics tod_sim_metrics_1000.csv --summary tod_sim_summary.csv
"""
...
def extract_slots(user_text: str) -> Dict[str, Any]:
    ...
    # destination only: (đến|tới|đi tới) Y
    ...
    # if a known place is mentioned without cues, do NOT guess origin/destination here (avoid swap)
    return slots
...
# Batch runner
# -------------------------
def run_batch(n: int, turns_out: str, metrics_out: str, summary_out: str, seed: int = 7):
    set_seed(seed)
    ...
    with open(turns_out, "w", newline="", encoding="utf-8") as ft, \
        ...
        for i in range(n):
            ...
            # one forced noisy slot in ~40% dialogues
            ...
            for _ in range(12):  # max system steps
                ...
                if sys_act.startswith("CONFIRM"):
                    ...
                    if wrong:
                        ...
                        tw.writerow({
                            ...
                            "extracted_slots":json.dumps(extract_slots(user_text),ensure_ascii=False),
                ...
                extracted = extract_slots(user_text)
...
def main():
    ...
    args = ap.parse_args()
    run_batch(args.n, args.turns, args.metrics, args.summary, seed=args.seed)
